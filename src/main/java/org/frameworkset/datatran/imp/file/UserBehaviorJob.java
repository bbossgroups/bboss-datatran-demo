...
public class UserBehaviorJob {
    ...
    private static Logger logger = LoggerFactory.getLogger(UserBehaviorJob.class);
    ...
    public static void main(String[] args) {
        UserBehaviorJob userBehaviorJob = new UserBehaviorJob();
        userBehaviorJob.run();
    }
    public void run() {
...
//        config.setBackupSuccessFileLiveTime( 10 * 60l);
        FtpConfig ftpConfig = new FtpConfig().setFtpIP("172.24.176.18").setFtpPort(22)
                ...
                .setRemoteFileDir("/mnt/c/sss/sss/21_sss/数据分析/1000").setSocketTimeout(600000L)
...
//                String[] messageArr = CSV_PARSER.parseLine(message);
                // 替换原 CSV_PARSER 使用方式
                String[] messageArr;
                try (CSVReader reader = new CSVReaderBuilder(new StringReader(message))
                        .withCSVParser(CSV_PARSER) // 复用已定义的 CSV_PARSER
                        .build()) {
                    messageArr = reader.readNext();
                } catch (IOException | CsvValidationException e) {
                    logger.warn("Failed to parse CSV line: {}", message, e);
                    context.setDrop(true);
                    return;
                }

                // 添加字段校验逻辑
                if (messageArr.length < 15) {
                    logger.warn("Invalid CSV line: {}", message);
                    context.setDrop(true);
                    return;
                }
                String mobile = messageArr[2];
                // 列字段不导入
                if ("user".equals(mobile)) {
                    logger.warn("列字段: {}", message);
                    context.setDrop(true);
                    return;
                }
                // 无效号码不导入
                if (mobile != null && mobile.contains("-")) {
                    logger.warn("无效号码: {}", message);
                    context.setDrop(true);
                    return;
                }
...
//                String now = DateFormatUtils.format(new Date(), "yyyy-MM-dd HH:mm:ss");
                ...
                context.addFieldValue("EVENT_KEY", eventKey);
...
//        importBuilder.setLastValueColumn("MOBILE");//手动指定数字增量查询字段，默认采用上面设置的sql语句中的增量变量名称作为增量查询字段的名称，指定以后就用指定的字段
        ...
        importBuilder.setLastValueStorePath("C:/data/cvs/"+JOB_ID+"_import");//todo 推荐绝对路径 记录上次采集的增量字段值的文件路径，作为下次增量（或者重启后）采集数据的起点，不同的任务这个路径要不一样
