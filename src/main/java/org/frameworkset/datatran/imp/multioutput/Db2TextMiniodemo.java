 ...
 * @version 1.0
 */
public class Db2TextMiniodemo {
	private static final Logger logger = LoggerFactory.getLogger(Db2TextMiniodemo.class);
	public static void main(String[] args){
		Db2TextMiniodemo dbdemo = new Db2TextMiniodemo();
...
//		//设置任务执行拦截器，可以添加多个，定时任务每次执行的拦截器
		...
		importBuilder.setLastValueStorePath("logdb2textminio_import");//记录上次采集的增量字段值的文件路径，作为下次增量（或者重启后）采集数据的起点，不同的任务这个路径要不一样
