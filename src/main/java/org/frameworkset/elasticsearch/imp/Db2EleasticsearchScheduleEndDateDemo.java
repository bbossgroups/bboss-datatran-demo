 ...
 * @version 1.0
 */
public class Db2EleasticsearchScheduleEndDateDemo {
	private static Logger logger = LoggerFactory.getLogger(Db2EleasticsearchScheduleEndDateDemo.class);
	public static void main(String args[]){
		Db2EleasticsearchScheduleEndDateDemo db2EleasticsearchDemo = new Db2EleasticsearchScheduleEndDateDemo();
...
//		importBuilder.setSql("select * from td_sm_log where LOG_OPERTIME > #[LOG_OPERTIME]");
		dbInputConfig.setSql("select * from td_sm_log where log_id > #[log_id]")
				...
				.setDbUrl("jdbc:mysql://192.168.137.1:3306/bboss?useUnicode=true&characterEncoding=utf-8&useSSL=false&rewriteBatchedStatements=true&allowPublicKeyRetrieval=true") //通过useCursorFetch=true启用mysql的游标fetch机制，否则会有严重的性能隐患，useCursorFetch必须和jdbcFetchSize参数配合使用，否则不会生效
				.setDbUser("root")
...
//		setFromfirst(true) 如果作业停了，作业重启后，重新开始采集数据
		...
		importBuilder.setLastValueStorePath("logtableend_import");//记录上次采集的增量字段值的文件路径，作为下次增量（或者重启后）采集数据的起点，不同的任务这个路径要不一样
