 ...
 * @version 1.0
 */
public class ES2SpecialTimestamSimpleDemo {
	private static Logger logger = LoggerFactory.getLogger(ES2SpecialTimestamSimpleDemo.class);
	public static void main(String[] args){
		...
		elasticsearchInputConfig.setDslFile("dsl2ndSqlFile.xml")//配置dsl和sql语句的配置文件
				.setDslName("ssimplescrollQuery") //指定从es查询索引文档数据的dsl语句名称，配置在dsl2ndSqlFile.xml中
...
//		setFromfirst(true) 如果作业停了，作业重启后，重新开始采集数据
		importBuilder.setStatusDbname("esss2custom");
		importBuilder.setLastValueStorePath("esss2timecustom_import");//记录上次采集的增量字段值的文件路径，作为下次增量（或者重启后）采集数据的起点，不同的任务这个路径要不一样
		importBuilder.setLastValueStoreTableName("esss2custom");//记录上次采集的增量字段值的表，可以不指定，采用默认表名increament_tab
