 ...
 * @version 1.0
 */
public class SFtpLog2FullfeatureHttpDslDemo {
	private static Logger logger = LoggerFactory.getLogger(SFtpLog2FullfeatureHttpDslDemo.class);
...
//		);
		...
		FtpConfig ftpConfig = new FtpConfig().setFtpIP("101.13.6.55").setFtpPort(32)
				.setFtpUser("3eee").setFtpPassword("1234")
				.setRemoteFileDir("/home/dddd/failLog")
...
//				.addHttpOutputConfig("jwtservice.http.health","/health") //服务监控检查地址
		...
		importBuilder.setLastValueStorePath("ftplogeshttpdsl_import");//记录上次采集的增量字段值的文件路径，作为下次增量（或者重启后）采集数据的起点，不同的任务这个路径要不一样
		//增量配置结束
