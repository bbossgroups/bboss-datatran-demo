package org.frameworkset.elasticsearch.imp.clickhouse;
/**
 ...
 * @version 1.0
 */
public class Clickhouse2Clickhousedemo {
	private static final Logger logger = LoggerFactory.getLogger(Clickhouse2Clickhousedemo.class);
	public static void main(String[] args){
		Clickhouse2Clickhousedemo dbdemo = new Clickhouse2Clickhousedemo();
...
//		dbdemo.scheduleImportData(dropIndice);
	...
	public void scheduleImportData(){
		...
		dbInputConfig.setDbName("source")
                ...
                .setDbUser("default")
                .setDbPassword("")
                .setBalance(DBConf.BALANCE_RANDOM)
...
//		importBuilder.setDateLastValueColumn("log_id");//手动指定日期增量查询字段，默认采用上面设置的sql语句中的增量变量名称作为增量查询字段的名称，指定以后就用指定的字段
		...
		importBuilder.setLastValueStorePath("logdb2clickhouse_import");//记录上次采集的增量字段值的文件路径，作为下次增量（或者重启后）采集数据的起点，不同的任务这个路径要不一样
