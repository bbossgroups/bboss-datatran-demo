package org.frameworkset.elasticsearch.imp.doris;
/**
 ...
 * @version 1.0
 */
public class Db2Dorisdemo {
	private static final Logger logger = LoggerFactory.getLogger(Db2Dorisdemo.class);
	public static void main(String[] args){
		Db2Dorisdemo dbdemo = new Db2Dorisdemo();
...
//		dbdemo.scheduleImportData(dropIndice);
	...
	public void scheduleImportData(){
		...
		dbOutputConfig.setDbName("target")
				.setDbDriver("com.mysql.cj.jdbc.Driver") //数据库驱动程序，必须导入相关数据库的驱动jar包
				.setDbUrl("jdbc:mysql://10.103.6.7:29030/testdb?allowPublicKeyRetrieval=true&") // doris query_port = 29030
				.setDbUser("root")
				.setDbPassword("")
				.setValidateSQL("select 1")
				.setUsePool(true)//是否使用连接池
...
//		importBuilder.setDateLastValueColumn("log_id");//手动指定日期增量查询字段，默认采用上面设置的sql语句中的增量变量名称作为增量查询字段的名称，指定以后就用指定的字段
		...
		importBuilder.setLastValueStorePath("logdb2doris_import");//记录上次采集的增量字段值的文件路径，作为下次增量（或者重启后）采集数据的起点，不同的任务这个路径要不一样
