 ...
 * @version 1.0
 */
public class TestKeyTimeMetrics1 {
	private static Logger logger = LoggerFactory.getLogger(TestKeyTimeMetrics1.class);
	public static void main(String[] args){
		...
		Metrics keyMetrics = new Metrics(Metrics.MetricsType_KeyTimeMetircs){
			...
			public void builderMetrics(){
                ...
                // 6. 定义时间窗口
				setTimeWindows(10);
                setTimeWindowType(MetricsConfig.TIME_WINDOW_TYPE_MONTH);
			}
			...
			public void persistent(Collection< KeyMetric> metrics) {
				metrics.forEach(keyMetric->{
					...
					esData.put("year", testKeyMetric.getYear());//指标年字段，例如2023
					esData.put("month", testKeyMetric.getMonth());//指标月期字段，例如2023-02
					...
					bulkProcessor.insertData("vops-testkeymonthmetrics",esData);//将指标结果保存到Elasticsearch指标表vops-testkeytimemetrics
